...
use nova_snark::frontend::{ConstraintSystem, LinearCombination, SynthesisError};
...
impl UInt32 {
  ...
  /// Bitwise NOT (`!self`) — free (per-bit negation).
  ...
  pub fn or<Scalar, CS>(&self, mut cs: CS, other: &Self) -> Result<Self, SynthesisError>
  where
    Scalar: PrimeField,
    CS: ConstraintSystem<Scalar>,
  {
    ...
    let bits = self
      ...
      .zip(other.bits.iter())
      .enumerate()
      .map(|(i, (a, b))| Boolean::or(cs.namespace(|| format!("or bit {}", i)), a, b))
      .collect::<Result<_, _>>()?;
    Ok(UInt32 { bits, value })
  }
