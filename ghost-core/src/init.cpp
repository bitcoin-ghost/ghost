...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-hazyncwitness=<dir>", "Emit a per-block JSON witness file to <dir> as each block connects, for the external Hazync zkVM prover (the archive-node bridge). Forces full-validation coin handling (disables SwiftSync). Observational only.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-ghostreaper-rejectdustflood", "Ghost Reaper: reject 1-in/1-out transactions whose sole non-OP_RETURN output sits at or below -ghostreaper-dustfloodthreshold (UTXO-flood spam) (default: follows -ghostreaper).", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    ...
    argsman.AddArg("-ghostreaper-dustfloodthreshold=<n>", strprintf("Sole-output value in sats at/below which a 1-in/1-out tx is treated as dust-flood spam (default: %u)", 330), ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-allowtiers=<csv>", "Ghost BUDS tier policy: comma-separated list of accepted transaction tiers, e.g. 0,1,2,3. T0=payments, T1=multisig/timelocks/HTLC, T2=small OP_RETURN, T3=inscriptions/runes/BRC-20/large data. Tiers not listed are rejected at mempool acceptance. (default: all tiers allowed — inert)", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-allowinscriptions=<0|1>", "Ghost BUDS tier policy: allow inscription-envelope transactions (default: 1). Set to 0 to reject them regardless of the allowed-tier set.", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-allowrunes=<0|1>", "Ghost BUDS tier policy: allow Runes (OP_RETURN OP_13) transactions (default: 1). Set to 0 to reject them regardless of the allowed-tier set.", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-allowbrc20=<0|1>", "Ghost BUDS tier policy: allow BRC-20 transactions (default: 1). Set to 0 to reject them regardless of the allowed-tier set.", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-maxopreturn=<n>", "Ghost BUDS tier policy: reject transactions whose largest OP_RETURN payload exceeds <n> bytes (default: no limit).", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-maxwitness=<n>", "Ghost BUDS tier policy: reject transactions with any input whose witness exceeds <n> bytes (default: no limit).", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-maxtxoutputs=<n>", "Ghost BUDS tier policy: reject transactions with more than <n> outputs (default: no limit).", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-maxtxsize=<n>", "Ghost BUDS tier policy: reject transactions larger than <n> virtual bytes (weight/4) (default: no limit).", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-ghostpolicy-minfeerate=<n>", "Ghost BUDS tier policy: reject transactions whose fee-rate is below <n> sat/vB (default: no floor).", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    ...
    const ArgsManager& args)
{
    ...
    // Hazync archive-node bridge: emit per-block zkVM prover witnesses if -hazyncwitness=<dir> is set.
    chainman.m_hazync_emitter = haze::HazyncWitnessEmitter::MaybeCreate(args);
