...
impl RoundManager {
    ...
    pub fn handle_share_proof(&self, proof: ShareProof) -> Result<(), ShareError> {
        ...
        // shares), C4 PoW re-verification below, and C5 dedup. Note the signature is verified by
        // the CALLERS, never here: `share_handler` drops foreign unsigned proofs (and skips
        // own-share gossip entirely, so gossip cannot reach this branch at all), and both
        // convergence lanes call `signature_is_valid` unconditionally. Any new caller of this
